...
// stores the image associated with the car model
function callApiAndStoreImages(models) {
    completedRequests = 0;

    function handleApiResponse(model, brand, imageUrl) {
        models[model].image = imageUrl;
        completedRequests++;

        if (completedRequests === 210) {
            console.log("image URLs have been loaded.");
            processData(models);
        }
    }

    function handleApiError(model, brand, error) {
        console.error(`Error fetching image for ${brand} ${model}:`, error);
        completedRequests++;
    ...
    {
        const obj = models[modelName];
        var model = obj.model;
        var brand = obj.make;
        /*make api call and store image*/
        ApiCallImage(model,brand,function(imageUrl){
            handleApiResponse(model, brand, imageUrl);
        }, function (error){
            handleApiError(model, brand, error);
        });
    }
}
